...
export function SignupForm() {
  ...
  const [state, formAction, isPending] = useActionState(signUpAction, INITIAL_AUTH_STATE);

  return (
    <form action={formAction} className="space-y-4">
      <div className="space-y-1">
        <label htmlFor="signup-email" className="block text-xs font-medium text-foreground">
          {t('fields.email')}
      ...
      <div className="space-y-1">
        <label htmlFor="signup-password" className="block text-xs font-medium text-foreground">
          {t('fields.password')}
          <span aria-hidden="true" className="ml-0.5 text-destructive">*</span>
        </label>
        <input
          ...
          className="w-full rounded-md border border-input bg-surface px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background"
        />
        <p className="text-[11px] text-muted-foreground">{t('fields.passwordHelp')}</p>
      </div>
